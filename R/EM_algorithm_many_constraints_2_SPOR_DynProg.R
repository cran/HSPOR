...
.EM_algorithm_many_constraints_2_SPOR_DynProg <- function(X,Y,deg,timeT,mat_param,sigma2,constraint,constraint_point,algo_direction,EM_nbIter=20){
  ...
  for(j in 1:EM_nbIter){
    ...
    FP_estimation <- .Fixed_point_method_many_constraints_2_SPOR_DynProg(X,Y,deg,timeT,sigma2,obs_weights,regimes_prob,constraint,constraint_point,algo_direction)
