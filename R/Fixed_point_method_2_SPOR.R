...
.Fixed_point_method_2_SPOR <- function(X,Y,deg,timeT,sigma2,obs_weights,regimes_prob,constraint,FP_nbIter=20){

  nbT <- length(timeT)

  for(p in 1:FP_nbIter){
    #vector parameters estimation
    M_mat <- .Jacobian_Matrix_2_SPOR(X,Y,deg,regimes_prob,timeT,sigma2,constraint)
